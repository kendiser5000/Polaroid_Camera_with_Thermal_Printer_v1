...
int8_t SendCmd(uint8_t command){
	...
	for (int i = 0; i < length; i++){
		UART1_OutChar(CommandList.SendCMD[command][i]);
...
int16_t BufferLengthCMD(void){
	...
	for (int i = 0; i < length; i++){
		UART1_OutChar(CommandList.SendCMD[getBuffLen][i]);
	}
	//DelayCam();
	...
	//ReadBufferCmd[8]  = (bufferLength & 0xFFFF) >> 8;
	//ReadBufferCmd[9]  = (bufferLength & 0xFFFF) >> 8;
	ReadBufferCmd[12] =  (bufferLength & 0xFFFF) >> 8;
	ReadBufferCmd[13] =  bufferLength & 0x00FF;
...
int8_t ReadBufferCMD(uint16_t length){
	...
	for (int i = 0; i < cmdlength; i++){
		UART1_OutChar(CommandList.SendCMD[readBuff][i]);
	}
	...
	// replace DEBUG_UART_OUTCHAR with how you store byte data
	for (int i = 0; i < length - 1; i ++){
		older = newer;
		//DEBUG_UART_OutHex((uint8_t)data);
		while(FiFo_Get(&data) == 0){
			count++;
			if (count == 0xFFFFF) return -1;
		};
		newer = data;
...
// take pic and transmit JPEG data to webserver?
