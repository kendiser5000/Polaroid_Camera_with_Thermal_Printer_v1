...
// MISO (pin 9) PD1
// SCK (pin 8) connected to PD3 (SSI0Clk)
// MOSI (pin 7) connected to PD1 (SSI0XDAT0/SSI0Tx)
// TFT_CS (pin 6) connected to PD2 (SSI0Fss)
// CARD_CS (pin 5) PD7????
// Data/Command (pin 4) connected to PD4 (GPIO)
// RESET (pin 3) connected to PD5 (GPIO)
...
//PD4
#define DC                      (*((volatile uint32_t *)0x4005B040))
#define DC_COMMAND              0
#define DC_DATA                 0x10
//PD5
#define RESET                   (*((volatile uint32_t *)0x4005B080))
#define RESET_LOW               0
...
// changed to work with different ports
// pinout
// PA2 -> PD3 (SCLK)
// PA3 -> PD2 (TFT_CS)
// PA4 -> PD1 (MISO)
// PA5 -> PD0 (MOSI)
// PA6 -> PD4 (Data/Command)
// PA7 -> PD5 (Reset)
//
...
void static commonInit(const unsigned char *cmdList) {
  ...
  GPIO_PORTD_DIR_R |= 0x34;             // make PD2,4,5 out
  GPIO_PORTD_AFSEL_R &= ~0x34;          // disable alt funct on PA3,6,7
  GPIO_PORTD_DEN_R |= 0x34;             // enable digital I/O on PA3,6,7
                                        // configure PA3,6,7 as GPIO
  GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0xFF00F0FF)+0x00000000;
  GPIO_PORTD_AMSEL_R &= ~0x34;          // disable analog functionality on PA3,6,7
  TFT_CS = TFT_CS_LOW;
  RESET = RESET_HIGH;
  Delay1ms(500);
  RESET = RESET_LOW;
  ...
  // initialize SSI0
  GPIO_PORTD_AFSEL_R |= 0x0E;           // enable alt funct on PD3,2,1
  GPIO_PORTD_DEN_R |= 0x0E;             // enable digital I/O on PA2,3,4
                                        // configure PA2,3,4 as SSI
  GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0xFFFF000F)+0x0000FFF0;
  GPIO_PORTD_AMSEL_R &= ~0x0E;          // disable analog functionality on PA2,3,4
